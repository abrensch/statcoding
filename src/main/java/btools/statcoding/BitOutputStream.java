...
public final class BitOutputStream extends DataOutputStream {
  ...
  // re-arrange the bits of a signed long to make it better suited for var-length coding
  ...
  /**
   * encode a positive long-array making use of the fact that it is sorted and unique.
   * This is done, starting with the most significant bit, by recursively encoding the
   ...
   * values and also benefits from clustering, because effectively a local typical distance
   * for the actual recursion level is used, not the global one over the whole array.
   *
  ...
  /**
   * Same as {@link #encodeUniqueSortedArray( long[] )}, but assuming
   * that the (sub-)size of the array is already known from context
  ...
  /**
   * Same as {@link #encodeUniqueSortedArray( long[], int, int, int )}, but assuming
   * that the most significant bit is known from context. This method calls itself
   * recursively down to subsize=1, where a fast shortcut kicks in to encode
   * the remaining bits of that remaining value-
   *
   * @param values  the array to encode
   * @param offset  position in this array where to start
   ...
   */
  protected void encodeUniqueSortedArray(long[] values, int offset, int subsize, long nextbit, long mask) throws IOException {
  ...
  private static void assertEquals( long l1, long l2 ) {
    if ( l1 != l2 ) throw new RuntimeException( "found " + l1 + " but expected " + l2 );
  }
